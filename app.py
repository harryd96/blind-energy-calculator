...
#   Adds CO₂‑equivalent savings and “trees‑saved” visual.
# -----------------------------------------------------------
...
# ───────────────────── Display Config ─────────────────────
st.set_page_config(page_title="Shard Blind Energy", layout="wide", page_icon="💡", initial_sidebar_state="expanded")
...
# ───────────────────── Defaults ─────────────────────
DEFAULT = dict(area=44_800, motor_old=120, motor_new=10, standby=0.5, moves=6,
...
with st.sidebar:
    ...
    area = st.number_input("Window Area (m²)", value=DEFAULT["area"], help="Total glazed area modelled (hotel floors excluded).")
...
# ───────────────────── Carbon Factors & Savings ─────────────────────
ELEC_CO2 = 0.233  # kg CO₂/kWh (UK grid 2024 avg)
...
FLIGHT_CO2 = 1.6  # t CO₂ London‑NYC return economy per passenger
...
st.table(pd.DataFrame({
    "Existing": [fmt(cool_old), cur(cost_cool_old), fmt(heat
